...
# file: wittyPiOff.sh
#
# Shut down system at specified time
...
if [ $local_hour -lt $now_hour -a $month_day -le $now_day ] ; then
	...
	echo ""
	echo "ERROR: shutdown time is in the past (hour)"
	echo ""
	...
	if [ $local_minute -lt $now_minute ] ; then
		...
		echo ""
		echo "ERROR: shutdown time is in the past (minute)"
		echo ""
...
#--------------------------
# Set system shutdown time
#--------------------------

set_shutdown_time $utc_date $utc_hour $utc_minute $utc_second 


echo ""
echo "system shutdown on $day_name $month_day $month_name $year $local_hour:$local_minute $tzone"
echo ""
